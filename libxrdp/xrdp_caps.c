...
xrdp_caps_process_rail(struct xrdp_rdp *self, struct stream *s, int len)
{
    ...
    LOG_DEVEL(LOG_LEVEL_TRACE, "Received [MS-RDPBCGR] TS_CONFIRM_ACTIVE_PDU - CAPSTYPE_RAIL "
...
xrdp_caps_process_confirm_active(struct xrdp_rdp *self, struct stream *s)
{
    ...
    for (index = 0; index < num_caps; index++)
    {
        ...
        switch (type)
        {
            ...
            case CAPSTYPE_FRAME_ACKNOWLEDGE:
                ...
                break;
            default:
                LOG_DEVEL(LOG_LEVEL_WARNING, "Received [MS-RDPBCGR] TS_CONFIRM_ACTIVE_PDU - TS_CAPS_SET "
    ...
    {
        LOG_DEVEL(LOG_LEVEL_INFO, "Client Capability: not enough orders "
...
xrdp_caps_send_demand_active(struct xrdp_rdp *self)
{
        ...
        self->client_info.multimon == 1)
    {
        ...
        {
            LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_caps_send_demand_active: error sending monitor layout pdu");
        }
    }
