...
process_enc_jpg(struct xrdp_encoder *self, XRDP_ENC_DATA *enc)
{
    ...
    for (index = 0; index < count; index++)
    {
        ...
        if (cx < 1 || cy < 1)
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "process_enc_jpg: error 1");
            continue;
        }
        ...
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "process_enc_jpg: error 2");
            return 1;
        }
        out_data = (char *) g_malloc(out_data_bytes + 256 + 2, 0);
        if (out_data == 0)
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "process_enc_jpg: error 3");
            return 1;
        }
        ...
        if (error < 0)
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "process_enc_jpg: jpeg error %d bytes %d",
                   error, out_data_bytes);
            g_free(out_data);
            return 1;
        }
        LOG_DEVEL(LOG_LEVEL_DEBUG, "jpeg error %d bytes %d", error, out_data_bytes);
...
proc_enc_msg(void *arg)
{
    ...
    if (self == 0)
    {
        LOG_DEVEL(LOG_LEVEL_INFO, "proc_enc_msg: self nil");
        return 0;
    }
    ...
    while (cont)
    {
        ...
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "proc_enc_msg: global term");
            break;
        }

        if (g_is_wait_obj_set(lterm_obj)) /* xrdp_mm term */
        {
            LOG_DEVEL(LOG_LEVEL_INFO, "proc_enc_msg: xrdp_mm term");
            break;
        }
    ...
    LOG_DEVEL(LOG_LEVEL_INFO, "proc_enc_msg: thread exit");
    return 0;
}
