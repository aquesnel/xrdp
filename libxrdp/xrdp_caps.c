...
xrdp_caps_send_demand_active(struct xrdp_rdp *self)
{
    ...
    {
        LOG_DBG("xrdp_caps_send_demand_active: xrdp_rdp_init failed");
        free_stream(s);
        return 1;
    }
    ...
    LOG_DBG("xrdp_caps_send_demand_active: Server Capability "
            "CAPSTYPE_SHARE "
    ...
    LOG_DBG("xrdp_caps_send_demand_active: sending PDUTYPE_DEMANDACTIVEPDU "
            "message with the server's capabilities");
    if (xrdp_rdp_send(self, s, PDUTYPE_DEMANDACTIVEPDU) != 0)
    {
        LOG_DBG("xrdp_caps_send_demand_active: xrdp_rdp_send failed");
        free_stream(s);
        return 1;
    }
    ...
    {
        LOG_DBG("xrdp_caps_send_demand_active: sending monitor layout pdu");
        if (xrdp_caps_send_monitorlayout(self) != 0)
        {
            LOG_DBG("xrdp_caps_send_demand_active: error sending monitor layout pdu");
        }
    }
