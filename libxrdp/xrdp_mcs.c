...
xrdp_mcs_send_cjcf(struct xrdp_mcs *self, int userid, int chanid)
{
    ...
    {
        free_stream(s);
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_mcs_send_cjcf: xrdp_iso_init failed");
        return 1;
    }
    ...
    */
    ...
    {
        free_stream(s);
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_mcs_send_cjcf: xrdp_iso_send failed");
        return 1;
    }
...
xrdp_mcs_recv(struct xrdp_mcs *self, struct stream *s, int *chan)
{
    ...
    while (1)
    {
        ...
        if (appid == MCS_CJRQ)
        {
            ...
            {
                LOG_DEVEL(LOG_LEVEL_WARNING, "xrdp_mcs_recv: xrdp_mcs_send_cjcf failed");
            }
    ...
    if (appid != MCS_SDRQ)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "Received [ITU-T T.125] DomainMCSPDU "
...
xrdp_mcs_send_aucf(struct xrdp_mcs *self)
{
    ...
    {
        free_stream(s);
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_mcs_send_aucf: xrdp_iso_init failed");
        return 1;
    }
    ...
    {
        free_stream(s);
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_mcs_send_aucf: xrdp_iso_send failed");
        return 1;
    }
...
xrdp_mcs_out_gcc_data(struct xrdp_sec *self)
{
    ...
    {
        LOG_DEVEL(LOG_LEVEL_DEBUG, "using 512 bit RSA key");
        /* [MS-RDPBCGR] TS_UD_HEADER */
    ...
    {
        LOG_DEVEL(LOG_LEVEL_DEBUG, "using 2048 bit RSA key");
        /* [MS-RDPBCGR] TS_UD_HEADER */
        out_uint16_le(s, SEC_TAG_SRV_CRYPT);  /* type */
    ...
    {
        LOG_DEVEL(LOG_LEVEL_DEBUG, "using no security");
        /* [MS-RDPBCGR] TS_UD_HEADER */
        out_uint16_le(s, SEC_TAG_SRV_CRYPT);  /* type*/
        out_uint16_le(s, 12);                 /* length */
        /* [MS-RDPBCGR] TS_UD_SC_SEC1 */
...
xrdp_mcs_send_connect_response(struct xrdp_mcs *self)
{
    ...
    {
        free_stream(s);
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_mcs_send_connect_response: xrdp_iso_send failed");
        return 1;
    }
...
xrdp_mcs_disconnect(struct xrdp_mcs *self)
{
    ...
    if (xrdp_iso_init(self->iso_layer, s) != 0)
    {
        ...
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_mcs_disconnect: xrdp_iso_init failed");
        return 1;
    }
    ...
    if (xrdp_iso_send(self->iso_layer, s) != 0)
    {
        free_stream(s);
        close_rdp_socket(self);
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_mcs_disconnect: xrdp_iso_send failed");
        return 1;
    }
