...
#define GRAPHICS_UPDATE_TYPE_TO_STR(type) \
    ((type) == RDP_UPDATE_ORDERS ? "UPDATETYPE_ORDERS" : \
     (type) == RDP_UPDATE_BITMAP ? "UPDATETYPE_BITMAP" : \
     (type) == RDP_UPDATE_PALETTE ? "UPDATETYPE_PALETTE" : \
     (type) == RDP_UPDATE_SYNCHRONIZE ? "UPDATETYPE_SYNCHRONIZE" : \
     "unknown" \
    )
