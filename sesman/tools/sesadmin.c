...
int main(int argc, char **argv)
{
    ...
    if (sock < 0)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "Socket open error, g_tcp_socket() failed");
        return 1;
    }
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "Connecting to %s:%s with user %s (%s)", serv, port, user, pass);

    if (0 != g_tcp_connect(sock, serv, port))
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "g_tcp_connect() error");
        return 1;
    }
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "libscp error connecting: %s %d", s->errstr, (int)e);
    }
