...
x_server_running_check_ports(int display)
{
    ...
    if (!x_running)
    {
        LOG(LOG_LEVEL_INFO, "Did not find a running X server at %s", text);
...
session_start_chansrv(char *username, int display)
{
    ...
    if (chansrv_pid == 0)
    {
        ...
        g_snprintf(exe_path, sizeof(exe_path), "%s/xrdp-chansrv",
                   XRDP_SBIN_PATH);
        LOG(LOG_LEVEL_DEBUG, "session_start_chansrv - exe_path: %s", exe_path);
                   ...
                   struct SCP_SESSION *s)
{
    ...
    if (pid == -1)
    {
        LOG(LOG_LEVEL_ERROR, "session_start_fork (pid %d): Failed to fork for scp with error: %s", g_getpid(), g_get_strerror());
    }
    else if (pid == 0)
    {
        ...
        {
            LOG(LOG_LEVEL_ERROR, "session_start_fork (pid %d): Failed to fork for the window manager with error: %s", g_getpid(), g_get_strerror());
        }
        else if (window_manager_pid == 0)
        {
            LOG(LOG_LEVEL_INFO, "session_start_fork (pid %d): waiting for Xserver", g_getpid());
            ...
            if (x_server_running(display))
            {
                ...
                LOG(LOG_LEVEL_DEBUG, "session_start_fork (pid %d): s->program not set", g_getpid());
                ...
                LOG(LOG_LEVEL_DEBUG, "session_start_fork (pid %d): g_cfg->enable_user_wm not set", g_getpid());
        ...
        {
            LOG(LOG_LEVEL_DEBUG, "session_start_fork (pid %d): forking for the Xserver", g_getpid());
            display_pid = g_fork(); /* parent becomes scp,
                                child becomes X */
            if (display_pid == -1)
            {
                LOG(LOG_LEVEL_ERROR, "session_start_fork (pid %d): Failed to fork for the X server with error: %s", g_getpid(), g_get_strerror());
            }
            else if (display_pid == 0) /* child */
            {
                ...
                LOG(LOG_LEVEL_DEBUG, "session_start_fork: setting Xserver environment variables");
                ...
                {
                    LOG(LOG_LEVEL_ERROR, "session_start_fork: error setting the xauth cookie. Exiting the Xserver xrdp-sesman fork.");
                    g_exit(1);
                }
            ...
            {
                LOG(LOG_LEVEL_DEBUG, "session_start_fork (pid %d): waiting for the Xserver and the creating the channel server", g_getpid());
                wait_for_xserver(display);
    ...
    {
        LOG(LOG_LEVEL_INFO, "session_start_fork: setting session info from pid %d", g_getpid());
        temp->item->pid = pid;
