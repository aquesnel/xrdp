...
xrdp_iso_recv_msg(struct xrdp_iso *self, struct stream *s, int *code, int *len)
{
    ...
    *len = 0;
    ...
    if (ver != 3)
    {
        ...
        LOG_DEVEL_HEXDUMP(LOG_LEVEL_TRACE, "[ITU-T T.123] TPKT header", s->data, 4);
        return 1;
    }
    ...
    if (*len == 255)
    {
        ...
        LOG_DEVEL_HEXDUMP(LOG_LEVEL_TRACE, "[ITU-T X.224] TPDU header", s->data + 4, 4);
        return 1;
    }
...
xrdp_iso_recv(struct xrdp_iso *self, struct stream *s)
{
    ...
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_iso_recv: xrdp_iso_recv_msg failed");
        return 1;
    }

    if (code != ISO_PDU_DT || len != 2)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_iso_recv only supports processing "
...
xrdp_iso_incoming(struct xrdp_iso *self)
{
    ...
    if (xrdp_iso_recv_msg(self, s, &code, &len) != 0)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_iso_incoming: xrdp_iso_recv_msg failed");
        return 1;
    }
    ...
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_iso_incoming: xrdp_iso_send_cc failed");
        return 1;
    }
