...
load_fuse_config(void)
{
    ...
    for (index = 0; index < items->count; index++)
    {
        ...
        else if (g_strcasecmp(item, "FileUmask") == 0)
        {
            ...
            LOG_DBG(LOG_LEVEL_INFO, "g_umask set to 0%o", g_umask);
        }
    }
    list_delete(items);
    list_delete(values);
...
xfuse_init(void)
{
    ...
    if (g_xfuse_inited)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "already inited");
        return 1;
    }

    if (g_ch != 0)
    {
        LOG_DBG(LOG_LEVEL_ERROR, "g_ch is not zero");
        return -1;
    }
...
int xfuse_create_share(tui32 device_id, const char *dirname)
{
        ...
        xfuse_init_xrdp_fs() == 0)
    {
        ...
        if (xinode == NULL)
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "xfs_add_entry() failed");
        }
        else
        {
...
{
    LOG_DBG(LOG_LEVEL_DEBUG, "entered: stream_id=%d data_bytes=%d", stream_id, data_bytes);

    struct req_list_item *rli;

    if ((rli = (struct req_list_item *) list_get_item(g_req_list, 0)) == NULL)
    {
        LOG_DBG(LOG_LEVEL_ERROR, "range error!");
        return -1;
    }

    LOG_DBG(LOG_LEVEL_DEBUG, "lindex=%d off=%d size=%d", rli->lindex, rli->off, rli->size);
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "completed all requests");
        return 0;
    }
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "requesting clipboard file data");
...
xfuse_add_clip_dir_item(const char *filename, int flags, int size, int lindex)
{
    ...
    if (xinode == NULL)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "failed to create file in xrdp filesystem");
        return -1;
    }
    xinode->size = size;
...
int xfuse_file_contents_size(int stream_id, int file_size)
{
    LOG_DBG(LOG_LEVEL_DEBUG, "entered: stream_id=%d file_size=%d", stream_id, file_size);
    return 0;
}
...
static int xfuse_init_lib(struct fuse_args *args)
{
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "fuse_parse_cmdline() failed");
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "fuse_mount() failed");
        fuse_opt_free_args(args);
        return -1;
    }
    ...
    if (g_se == 0)
    {
        LOG_DBG(LOG_LEVEL_ERROR, "fuse_lowlevel_new() failed");
        fuse_unmount(g_mount_point, g_ch);
        g_ch = 0;
        fuse_opt_free_args(args);
        return -1;
                                 ...
                                 const struct file_attr *fattr)
{
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "inode %ld is not valid", fip->pinum);
    }
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "parent_inode=%ld name=%s", fip->pinum, name);
        ...
        if (xinode == NULL)
        {
            ...
            LOG_DBG(LOG_LEVEL_DEBUG, "Creating name=%s in parent=%ld in xrdp_fs",
                      name, fip->pinum);
            xinode = xfs_add_entry(g_xfs, fip->pinum, name, fattr->mode);
            if (xinode == NULL)
            {
                LOG_DBG(LOG_LEVEL_ERROR, "xfs_add_entry() failed");
            }
            else
            {
...
void xfuse_devredir_cb_enum_dir_done(struct state_dirscan *fip,
                                     enum NTSTATUS IoStatus)
{
    LOG_DBG(LOG_LEVEL_DEBUG, "fip=%p IoStatus=0x%x", fip, IoStatus);
                                    ...
                                    const struct file_attr *file_info)
{
    ...
    if (IoStatus != STATUS_SUCCESS)
    {
        switch (IoStatus)
        {
            ...
            default:
                LOG_DBG(LOG_LEVEL_INFO, "Error code %08x - fallthrough", (int) IoStatus);
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "parent inode %ld is not valid", fip->pinum);
        fuse_reply_err(fip->req, ENOENT);
    }
    else
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "parent_inode=%ld name=%s", fip->pinum, fip->name);
        ...
        if (xinode != NULL)
        {
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "inode=%ld name=%s already exists in xrdp_fs as %ld",
                ...
                if (xfs_get_file_open_count(g_xfs, xinode->inum) > 0)
                {
                     ...
                     */
                    LOG_DBG(LOG_LEVEL_DEBUG, "inode=%ld is open - "
                              "preferring local attributes", xinode->inum);
                }
                else
                {
                    LOG_DBG(LOG_LEVEL_DEBUG, "Updating attributes of inode=%ld", xinode->inum);
            ...
            else
            {
                ...
                LOG_DBG(LOG_LEVEL_DEBUG, "inode=%ld name=%s of different type in xrdp_fs"
                          " - removing",
                                   ...
                                   tui32 DeviceId, tui32 FileId)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if (xinode == NULL)
            {
                LOG_DBG(LOG_LEVEL_ERROR, "Out of memory!");
                fuse_reply_err(fip->req, ENOMEM);
            }
            else
            {
                                 ...
                                 tui32 DeviceId, tui32 FileId)
{
    ...
    else
    {
        ...
        else
        {
            ...
            LOG_DBG(LOG_LEVEL_DEBUG, "sending fuse_reply_open(); "
                                 ...
                                 const char *buf, size_t length)
{
    if (IoStatus != STATUS_SUCCESS)
    {
        LOG_DBG(LOG_LEVEL_ERROR, "Read NTSTATUS is %d", (int) IoStatus);
        fuse_reply_err(fip->req, EIO);
    }
    else
    {
                                 ...
                                 size_t length)
{
    ...
    if (IoStatus != STATUS_SUCCESS)
    {
        LOG_DBG(LOG_LEVEL_ERROR, "Write NTSTATUS is %d", (int) IoStatus);
        fuse_reply_err(fip->req, EIO);
    }
    else
    {
        ...
        {
            LOG_DBG(LOG_LEVEL_ERROR, "inode %ld is invalid", fip->inum);
        }
    }
...
static void xfuse_cb_lookup(fuse_req_t req, fuse_ino_t parent, const char *name)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "looking for parent=%ld name=%s", parent, name);
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "inode %ld is not valid", parent);
    ...
    else
    {
        if (!parent_xinode->is_redirected)
        {
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "found entry for parent=%ld name=%s",
                          parent, name);
        ...
        else
        {
            ...
            else
            {
                ...
                LOG_DBG(LOG_LEVEL_DEBUG, "Looking up %s in %s on %d", name, cptr,
                ...
                {
                    LOG_DBG(LOG_LEVEL_ERROR, "failed to send devredir_lookup_entry() cmd");
                            ...
                            struct fuse_file_info *fi)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "req=%p ino=%ld", req, ino);

    /* if ino is not valid, just return */
    if ((xino = xfs_get(g_xfs, ino)) == NULL)
    {
        LOG_DBG(LOG_LEVEL_ERROR, "inode %ld is not valid", ino);
        fuse_reply_err(req, ENOENT);
    }
    else
    {
                             ...
                             off_t off, struct fuse_file_info *fi)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "req=%p inode=%ld size=%zd offset=%lld", req, ino, size, (long long) off);
                           ...
                           const char *name, mode_t mode)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "entered: parent_inode=%ld name=%s", parent, name);
                            ...
                            const char *name)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "entered: parent=%ld name=%s", parent, name);
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "did not find file with pinode=%ld name=%s", parent, name);
        fuse_reply_err(req, ENOENT);
    }
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "cannot rmdir; directory is not empty");
    ...
    else if (!xinode->is_redirected)
    {
        ...
        LOG_DBG(LOG_LEVEL_DEBUG, "LK_TODO: this is still a TODO");
    ...
    else
    {
        ...
        else
        {
            ...
            {
                LOG_DBG(LOG_LEVEL_ERROR, "failed to send devredir_rmdir_or_file() cmd");
                fuse_reply_err(req, EREMOTEIO);
                free(fip);
            }
                            ...
                            fuse_ino_t new_parent, const char *new_name)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "entered: old_parent=%ld old_name=%s new_parent=%ld new_name=%s",
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "did not find file with pinode=%ld name=%s",
                  old_parent, old_name);
        fuse_reply_err(req, ENOENT);
    }
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "inode %ld is not valid", new_parent);
        fuse_reply_err(req, ENOENT);
    }
    ...
    else
    {
        ...
        else
        {
            ...
            {
                LOG_DBG(LOG_LEVEL_ERROR, "failed to send devredir_file_rename() cmd");
                fuse_reply_err(req, EREMOTEIO);
                free(fip);
            }
            free(old_full_path);
                                     ...
                                     struct fuse_file_info *fi)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "entered: parent_ino=%ld name=%s mode=%o type=%s",
    ...
    else
    {
        ...
        else
        {
            ...
            else
            {
                ...
                {
                    LOG_DBG(LOG_LEVEL_ERROR, "failed to send devredir_file_create() cmd");
                    fuse_reply_err(req, EREMOTEIO);
                    free(fip);
                }
                free(full_path);
                          ...
                          struct fuse_file_info *fi)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "entered: ino=%ld", ino);
    ...
    else if (xinode->mode & S_IFDIR)
    {
        ...
        LOG_DBG(LOG_LEVEL_DEBUG, "reading/writing a dir not allowed!");
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "Invalid access mode specified");
        fuse_reply_err(req, EINVAL);
    }
    else if (!xinode->is_redirected)
    {
    ...
    else
    {
        ...
        else
        {
            ...
            {
                LOG_DBG(LOG_LEVEL_ERROR, "failed to send devredir_file_open() cmd");
                fuse_reply_err(req, EREMOTEIO);
                free(fip);
            }
            free(full_path);
                             ...
                             fuse_file_info *fi)
{
    ...
    else
    {
        ...
        {
            LOG_DBG(LOG_LEVEL_ERROR, "failed to send devredir_close_file() cmd");
            fuse_reply_err(req, EREMOTEIO);
            free(fip);
        }
                          ...
                          off_t off, struct fuse_file_info *fi)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "want_bytes %zd bytes at off %lld", size, (long long) off);
    ...
    else if (fh->is_loc_resource)
    {
        ...
        LOG_DBG(LOG_LEVEL_DEBUG, "target file is in .clipboard dir");

        if ((xinode = xfs_get(g_xfs, ino)) == NULL)
        {
            LOG_DBG(LOG_LEVEL_ERROR, "ino does not exist in xrdp_fs");
            fuse_reply_buf(req, 0, 0);
            return;
        }
                           ...
                           size_t size, off_t off, struct fuse_file_info *fi)
{
    ...
    if ((fh = (XFUSE_HANDLE *)fi->fh) == NULL)
    {
        LOG_DBG(LOG_LEVEL_ERROR, "file handle fi->fh is NULL");
        fuse_reply_err(req, EINVAL);
    }
    else if (fh->is_loc_resource)
    {
        /* target file is in .clipboard dir */
        LOG_DBG(LOG_LEVEL_DEBUG, "THIS IS STILL A TODO!");
        fuse_reply_err(req, EROFS);
    }
    else
    {
...
static void xfuse_cb_fsync(fuse_req_t req, fuse_ino_t ino, int datasync,
                           struct fuse_file_info *fi)
{
    LOG_DBG(LOG_LEVEL_DEBUG, "#################### entered: ino=%ld datasync=%d", ino, datasync);
    LOG_DBG(LOG_LEVEL_DEBUG, "function not required");
    fuse_reply_err(req, EINVAL);
}
#endif
                             ...
                             int to_set, struct fuse_file_info *fi)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "entered to_set=0x%x", to_set);
              ...
              (attr->st_mode & ~(0777 | S_IFDIR | S_IFREG)) != 0)
    {
        ...
        LOG_DBG(LOG_LEVEL_ERROR, "Asking for invalid mode bits 0%o to be set", attr->st_mode);
        fuse_reply_err(req, EINVAL);
    }
    else    
    {
                                  ...
                                  struct fuse_file_info *fi)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "inode=%ld", ino);
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "did not find entry; redirecting call to devredir");
        ...
        {
            const char      *cptr;
            LOG_DBG(LOG_LEVEL_DEBUG, "dev_id=%d ino=%ld full_path=%s",
            ...
            {
                LOG_DBG(LOG_LEVEL_ERROR, "failed to send devredir_get_dir_listing() cmd");
