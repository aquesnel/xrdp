...
clipboard_init(void)
{
    ...
    LOG_DBG(LOG_LEVEL_INFO, "clipboard_init:");
    ...
    if (rv == 0)
    {
        ...
        {
            LOG_DBG(LOG_LEVEL_ERROR, "clipboard_init: XInternAtom failed");
            rv = 3;
        }
    }
    ...
    if (rv == 0)
    {
        ...
        {
            LOG_DBG(LOG_LEVEL_ERROR, "clipboard_init: no xfixes");
            rv = 5;
        }
    }
    ...
    if (rv == 0)
    {
        ...
        {
            LOG_DBG(LOG_LEVEL_ERROR, "clipboard_init: g_image_bmp_atom was "
                  "not allocated");
        }
    ...
    if (rv == 0)
    {
        ...
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_init: data out, sending "
              "CB_CLIP_CAPS (clip_msg_id = 1)");
        rv = send_channel_data(g_cliprdr_chan_id, s->data, size);
        if (rv != 0)
        {
            LOG_DBG(LOG_LEVEL_ERROR, "clipboard_init: send_channel_data failed "
                  "rv = %d", rv);
            rv = 4;
        }
    }
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "xrdp-chansrv: clipboard_init: error on exit");
    }
...
{
    LOG_DBG(LOG_LEVEL_INFO, "clipboard_deinit:");
    if (g_wnd != 0)
    {
...
clipboard_send_data_request(int format_id)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_data_request:");
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_data_request: %d", format_id);
    ...
    s_mark_end(s);
    size = (int)(s->end - s->data);
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_data_request: data out, sending "
...
clipboard_send_format_ack(void)
{
    ...
    s_mark_end(s);
    size = (int)(s->end - s->data);
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_format_ack: data out, sending "
...
clipboard_send_format_announce(int xrdp_clip_type)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_format_announce:");
    make_stream(s);
    init_stream(s, 8192);
    ...
    if (g_cliprdr_flags & CB_USE_LONG_FORMAT_NAMES)
    {
        ...
        {
            case XRDP_CB_FILE:
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_format_announce: XRDP_CB_FILE");
                ...
                break;
            case XRDP_CB_BITMAP:
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_format_announce: XRDP_CB_BITMAP");
                ...
                break;
            case XRDP_CB_TEXT:
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_format_announce: XRDP_CB_TEXT");
                /* canned response for "bitmap" */
                ...
                break;
            default:
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_format_announce: unknown "
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_format_announce: data out, sending "
...
clipboard_send_data_response_for_image(const char *data, int data_size)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_data_response_for_image: data_size %d",
                data_size);
    make_stream(s);
...
clipboard_send_data_response_for_text(const char *data, int data_size)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_data_response_for_text: data_size %d",
                 data_size);
    //g_hexdump(data, data_size);
    num_chars = g_mbstowcs(0, data, 0);
    if (num_chars < 0)
    {
        LOG_DBG(LOG_LEVEL_ERROR, "clipboard_send_data_response_for_text: "
             "bad string");
        num_chars = 0;
    }
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_data_response_for_text: data_size %d "
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "clipboard_send_data_response_for_text: error "
    ...
    s_mark_end(s);
    size = (int)(s->end - s->data);
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_data_response_for_text: data out, "
...
{
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_data_response:");
    if (data != 0)
    {
        ...
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_send_data_response: unknown "
                       "xrdp_clip_type %d", xrdp_clip_type);
        }
    }
    else
    {
        LOG_DBG(LOG_LEVEL_ERROR, "clipboard_send_data_response: data is nil");
    }
    return 0;
}
...
clipboard_set_selection_owner(void)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_set_selection_owner:");
...
clipboard_provide_selection_c2s(XSelectionRequestEvent *req, Atom type)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_provide_selection_c2s: bytes %d",
    ...
    else
    {
        ...
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_provide_selection_c2s: start INCR property %s "
                            ...
                            char *data, int length)
{
    ...
    bytes *= length;
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_provide_selection: bytes %d", bytes);
                                  ...
                                  int clip_msg_len)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_format_announce: "
          "CLIPRDR_FORMAT_ANNOUNCE");
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_format_announce %d", clip_msg_len);
    ...
    while (clip_msg_len > 3)
    {
        ...
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_format_announce: formatId 0x%8.8x "
        ...
        if (g_num_formatIds > 15)
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_format_announce: max formats");
        }
            ...
            (g_clip_s2c.incr_in_progress == 0))
    {
        ...
        {
            LOG_DBG(LOG_LEVEL_ERROR, "clipboard_process_format_announce: "
...
clipboard_process_format_ack(struct stream *s, int clip_msg_status,
                             int clip_msg_len)
{
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_format_ack: CLIPRDR_FORMAT_ACK");
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_format_ack:");
    return 0;
}
...
clipboard_send_data_response_failed(void)
{
    ...
    LOG_DBG(LOG_LEVEL_ERROR, "clipboard_send_data_response_failed:");
    make_stream(s);
    init_stream(s, 64);
    out_uint16_le(s, CB_FORMAT_DATA_RESPONSE); /* 5 CLIPRDR_DATA_RESPONSE */
                               ...
                               int clip_msg_len)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_data_request: "
          "CLIPRDR_DATA_REQUEST");
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_data_request:");
    LOG_DBG(LOG_LEVEL_DEBUG, "  %d", g_clip_s2c.xrdp_clip_type);
    ...
    switch (requestedFormatId)
    {
        case CB_FORMAT_FILE: /* 0xC0BC */
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_data_request: CB_FORMAT_FILE");
        ...
        case CB_FORMAT_DIB: /* 0x0008 */
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_data_request: CB_FORMAT_DIB");
        ...
        case CB_FORMAT_UNICODETEXT: /* 0x000D */
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_data_request: CB_FORMAT_UNICODETEXT");
            ...
            break;
        default:
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_data_request: unknown type %d",
                        requestedFormatId);
        ...
        int clip_msg_len)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_data_response_for_image: "
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_data_response_for_image: calling "
                                ...
                                int clip_msg_len)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_data_response:");
    lxev = &g_saved_selection_req_event;
    ...
    if (g_clip_c2s.xrdp_clip_type == XRDP_CB_FILE)
    {
        ...
        {
            LOG_DBG(LOG_LEVEL_ERROR, "clipboard_process_data_response: error");
        }
                            ...
                            int clip_msg_len)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_clip_caps:");
    ...
    for (index = 0; index < cCapabilitiesSets; index++)
    {
        ...
        switch (capabilitySetType)
        {
            case CB_CAPSTYPE_GENERAL:
                ...
                break;
            default:
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_process_clip_caps: unknown "
...
ss_part(char *data, int data_bytes)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "ss_part: data_bytes %d read_bytes_done %d "
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "ss_part: incr_in_progress set");
        return 0;
    }
    if (g_clip_c2s.read_bytes_done <= g_clip_c2s.incr_bytes_done)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "ss_part: read_bytes_done < incr_bytes_done");
        return 0;
    }
...
ss_end(void)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "ss_end:");
    g_clip_c2s.doing_response_ss = 0;
    g_clip_c2s.in_request = 0;

    if (g_clip_c2s.incr_in_progress)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "ss_end: incr_in_progress set");
        return 0;
    }
    if (g_clip_c2s.read_bytes_done <= g_clip_c2s.incr_bytes_done)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "ss_end: read_bytes_done < incr_bytes_done");
        return 0;
    }
    data = g_clip_c2s.data + g_clip_c2s.incr_bytes_done;
    data_bytes = g_clip_c2s.read_bytes_done - g_clip_c2s.incr_bytes_done;
...
ss_start(char *data, int data_bytes, int total_bytes)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "ss_start: data_bytes %d total_bytes %d",
                  ...
                  int total_length)
{
    ...
    if (!g_clip_up)
    {
        LOG_DBG(LOG_LEVEL_ERROR, "aborting clipboard_data_in - clipboard has not "
            "been initialized");
        /* we return 0 here to indicate no protocol problem occurred */
        return 0;
    }

    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_data_in: chan_id %d "
    ...
    if (g_clip_c2s.doing_response_ss)
    {
        ...
        if ((chan_flags & 3) == 2)
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_data_in: calling ss_end");
            ss_end();
        }
        return 0;
    }
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_data_in: clip_msg_id %d "
             "clip_msg_status %d clip_msg_len %d",
             clip_msg_id, clip_msg_status, clip_msg_len);
    rv = 0;

    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_data_in: %d", clip_msg_id);
    switch (clip_msg_id)
    {
        /* sent by client or server when its local system clipboard is   */
        /* updated with new clipboard data; contains Clipboard Format ID */
        ...
        case CB_FILECONTENTS_RESPONSE: /* 9 */
            ...
            break;
        default:
            LOG_DBG(LOG_LEVEL_ERROR, "clipboard_data_in: unknown clip_msg_id %d", clip_msg_id);
            break;
    }
...
clipboard_event_selection_owner_notify(XEvent *xevent)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_owner_notify: 0x%lx", lxevent->owner);
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_owner_notify: "
          "window %ld subtype %d owner %ld g_wnd %ld",
          lxevent->window, lxevent->subtype, lxevent->owner, g_wnd);

    if (lxevent->owner == g_wnd)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_owner_notify: matches g_wnd");
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_owner_notify: skipping, "
              "owner == g_wnd");
        g_got_selection = 1;
        return 0;
    }
                              ...
                              int *n_items, char **xdata, int *xdata_size)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_get_window_property:");
    LOG_DBG(LOG_LEVEL_DEBUG, "  prop %ld name %s", prop, get_atom_text(prop));
    lxdata = 0;
    ltype = 0;
...
clipboard_event_selection_notify(XEvent *xevent)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_notify:");
    data_size = 0;
    n_items = 0;
    fmt = 0;
    got_string = 0;
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "clipboard_event_selection_notify: clip could "
              "not be converted");
        rv = 1;
    }
    ...
    if (rv == 0)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_notify: wnd 0x%lx prop %s",
                    lxevent->requestor,
        ...
        if (rv != 0)
        {
            LOG_DBG(LOG_LEVEL_ERROR, "clipboard_event_selection_notify: "
        ...
        if (type == g_incr_atom)
        {
            /* nothing more to do here, the data is coming in through
               PropertyNotify */
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_notify: type is INCR "
        ...
        if (lxevent->selection == g_clipboard_atom)
        {
            if (lxevent->target == g_targets_atom)
            {
                ...
                if ((type == XA_ATOM) && (fmt == 32))
                {
                    ...
                    {
                        atom = atoms[index];
                        LOG_DBG(LOG_LEVEL_DEBUG,
                              "clipboard_event_selection_notify: 0x%lx %s 0x%lx",
                              atom, get_atom_text(atom), XA_STRING);
                        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_notify: 0x%lx %s",
                        ...
                        {
                            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_notify: file");
                            got_file_atom = atom;
                        }
                        else
                        {
                            LOG_DBG(LOG_LEVEL_ERROR, "clipboard_event_selection_notify: unknown atom 0x%lx", atom);
                        }
                    }
                }
                else
                {
                    LOG_DBG(LOG_LEVEL_ERROR, "clipboard_event_selection_notify: error, "
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_notify: UTF8_STRING "
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_notify: XA_STRING "
                      "data_size %d", data_size);
                if ((g_clip_s2c.incr_in_progress == 0) && (data_size > 0))
                {
                    g_free(g_clip_s2c.data);
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_notify: image/bmp "
                      "data_size %d", data_size);
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_notify: text/uri-list "
                      "data_size %d", data_size);
                if ((g_clip_s2c.incr_in_progress == 0) && (data_size > 0))
                {
                    g_free(g_clip_s2c.data);
...
clipboard_event_selection_request(XEvent *xevent)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_request: 0x%lx", lxev->property);
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_request: g_wnd %ld, "
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_request: "
              "lxev->property is None");
    }
    else if (lxev->target == g_targets_atom)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_request: g_targets_atom");
        ...
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "  reporting image/bmp");
            atom_buf[atom_count] = g_image_bmp_atom;
            atom_count++;
        }
        if (clipboard_find_format_id(g_file_format_id) >= 0)
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "  reporting text/uri-list");
            atom_buf[atom_count] = g_file_atom1;
            atom_count++;
            LOG_DBG(LOG_LEVEL_DEBUG, "  reporting x-special/gnome-copied-files");
            atom_buf[atom_count] = g_file_atom2;
            atom_count++;
        }
        atom_buf[atom_count] = 0;
        LOG_DBG(LOG_LEVEL_DEBUG, "  reporting %d formats", atom_count);
    ...
    else if (lxev->target == g_multiple_atom)
    {
        ...
        if (clipboard_get_window_property(lxev->requestor, lxev->property,
                                          ...
                                          &xdata_size) == 0)
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_request: g_multiple_atom "
                  "n_items %d", n_items);
            /* todo */
            g_free(xdata);
        }
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_request: image/bmp");
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_request: g_file_atom1");
        if ((g_clip_c2s.type == lxev->target) && g_clip_c2s.converted)
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_request: -------------------------------------------");
            clipboard_provide_selection_c2s(lxev, lxev->target);
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_request: g_file_atom2");
        if ((g_clip_c2s.type == lxev->target) && g_clip_c2s.converted)
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_request: -------------------------------------------");
            clipboard_provide_selection_c2s(lxev, lxev->target);
...
{
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_selection_clear:");
    return 0;
}
...
clipboard_event_property_notify(XEvent *xevent)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_property_notify: PropertyNotify .window %ld "
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_property_notify: INCR PropertyDelete");
        ...
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_property_notify: INCR error");
            return 0;
        }
        ...
        g_clip_c2s.incr_bytes_done += data_bytes;
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_property_notify: data_bytes %d", data_bytes);
        ...
        if (data_bytes < 1)
        {
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_property_notify: INCR done");
            g_clip_c2s.incr_in_progress = 0;
    ...
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_property_notify: INCR PropertyNewValue");
        ...
        if (bytes_left <= 0)
        {
            ...
            {
                LOG_DBG(LOG_LEVEL_ERROR, "clipboard_event_property_notify: error unknown type %ld",
                           g_clip_s2c.type);
                clipboard_send_data_response_failed();
            }
        ...
        else
        {
            ...
            LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_event_property_notify: new_data_len %d", new_data_len);
...
clipboard_xevent(void *xevent)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_xevent: event detected");
    ...
    switch (lxevent->type)
    {
        case SelectionNotify:
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "outbound clipboard is restricted because of config");
                return 1;
            }
            break;
        ...
        case PropertyNotify:
            ...
            break;
        case UnmapNotify:
            LOG_DBG(LOG_LEVEL_DEBUG, "chansrv::clipboard_xevent: got UnmapNotify");
            break;
        case ClientMessage:
            LOG_DBG(LOG_LEVEL_DEBUG, "chansrv::clipboard_xevent: got ClientMessage");
            break;
        default:
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_xevent: got XFixesSetSelectionOwnerNotify");
            ...
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_xevent: got XFixesSelectionWindowDestroyNotify");
                break;
            }
            if (lxevent->type == g_xfixes_event_base +
                    XFixesSelectionClientCloseNotify)
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "clipboard_xevent: got XFixesSelectionClientCloseNotify");
                break;
            }
