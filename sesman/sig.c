...
sig_sesman_shutdown(int sig)
{
    ...
    if (g_getpid() != g_pid)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "g_getpid() [%d] differs from g_pid [%d]", (g_getpid()), g_pid);
        return;
    }

    LOG_DBG(LOG_LEVEL_DEBUG, " - getting signal %d pid %d", sig, g_getpid());
...
sig_handler_thread(void *arg)
{
    ...
    do
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "calling sigwait()");
        ...
        switch (recv_signal)
        {
            case SIGHUP:
                ...
                LOG_DBG(LOG_LEVEL_DEBUG, "sesman received SIGHUP");
                //return 0;
                break;
            case SIGCHLD:
                /* a session died */
                LOG_DBG(LOG_LEVEL_DEBUG, "sesman received SIGCHLD");
                sig_sesman_session_end(SIGCHLD);
                break;
            case SIGINT:
                /* we die */
                LOG_DBG(LOG_LEVEL_DEBUG, "sesman received SIGINT");
                sig_sesman_shutdown(recv_signal);
                break;
            case SIGTERM:
                /* we die */
                LOG_DBG(LOG_LEVEL_DEBUG, "sesman received SIGTERM");
                sig_sesman_shutdown(recv_signal);
                break;
        }
    }
