...
xrdp_rdp_read_config(struct xrdp_client_info *client_info)
{
    ...
    LOG(LOG_LEVEL_DEBUG, "Reading config file %s", cfg_file);
...
xrdp_rdp_recv(struct xrdp_rdp *self, struct stream *s, int *code)
{
    ...
    if (s->next_packet == 0 || s->next_packet >= s->end)
    {
        ...
        if (error != 0)
        {
            LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_rdp_recv: xrdp_sec_recv failed");
            return 1;
        }
        ...
        if ((chan != MCS_GLOBAL_CHANNEL) && (chan > 0))
        {
            if (chan > MCS_GLOBAL_CHANNEL)
            {
                ...
                {
                    LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_rdp_recv: xrdp_channel_process failed");
                }
            }
            else
            {
...
xrdp_rdp_send(struct xrdp_rdp *self, struct stream *s, int pdu_type)
{
    ...
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_rdp_send: xrdp_sec_send failed");
        return 1;
    }
                   ...
                   int data_pdu_type)
{
    ...
    if (xrdp_sec_send(self->sec_layer, s, MCS_GLOBAL_CHANNEL) != 0)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_rdp_send_data: xrdp_sec_send failed");
        return 1;
    }
                       ...
                       int data_pdu_type)
{
    ...
    while (cont)
    {
        ...
        if ((compression != 0) && (no_comp_len > header_bytes + 16))
        {
            ...
            {
                LOG_DEVEL(LOG_LEVEL_DEBUG, 
                          "xrdp_rdp_send_fastpath: compress_rdp failed, "
        ...
        {
            LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_rdp_send_fastpath: xrdp_sec_send_fastpath failed");
            return 1;
        }
...
xrdp_rdp_send_data_update_sync(struct xrdp_rdp *self)
{
    ...
    else /* slowpath */
    {
        ...
        {
            LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_rdp_send_data_update_sync: xrdp_rdp_init_data failed");
            free_stream(s);
            return 1;
        }
    ...
    else /* slowpath */
    {
        ...
        {
            LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_rdp_send_data_update_sync: xrdp_rdp_send_data failed");
            free_stream(s);
            return 1;
        }
    }
