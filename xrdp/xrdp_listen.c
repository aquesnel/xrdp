...
xrdp_listen_create_pro_done(struct xrdp_listen *self)
{
    ...
    {
        LOG(LOG_LEVEL_ERROR,"Failure creating pro_done_event");
    }
    ...
    while (cont)
    {
            ...
            (mode == TRANS_MODE_TCP6))
        {
            ...
            if (startup_params->tcp_send_buffer_bytes > 0)
            {
                ...
                {
                    LOG(LOG_LEVEL_ERROR, "error setting send buffer");
                }
                else
                {
                    if (g_sck_get_send_buffer_bytes(ltrans->sck, &bytes) != 0)
                    {
                        LOG(LOG_LEVEL_ERROR, "error getting send "
                                    "buffer");
                    }
                    else
                    {
            ...
            if (startup_params->tcp_recv_buffer_bytes > 0)
            {
                ...
                {
                    LOG(LOG_LEVEL_ERROR, "error setting recv buffer");
                }
                else
                {
                    if (g_sck_get_recv_buffer_bytes(ltrans->sck, &bytes) != 0)
                    {
                        LOG(LOG_LEVEL_ERROR, "error getting recv "
                                    "buffer");
                    }
                    else
                    {
