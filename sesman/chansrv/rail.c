    ...
    if (enabled)
    {
        ...
        else
        {
            ...
            if (transient_for > 0)
            {
                ...
                LOG_DEVEL(LOG_LEVEL_DEBUG, "Enabling owner window 0x%8.8x", transient_for);
                LOG_DEVEL(LOG_LEVEL_TRACE, 
                          "calling XRaiseWindow for window 0x%8.8x", transient_for);
...
rail_xevent(void *xevent)
{
    ...
    switch (lxevent->type)
    {
        case PropertyNotify:
            ...
            {
                LOG_DEVEL(LOG_LEVEL_WARNING, 
                          "Ignorning [XEvent] PropertyNotify, window 0x%8.8x "
