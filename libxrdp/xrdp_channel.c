...
xrdp_channel_get_item(struct xrdp_channel *self, int channel_id)
{
    ...
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_channel_get_item: No channel initialized");
        return NULL ;
    }
                  ...
                  int total_data_len, int flags)
{
    ...
    if (channel == NULL)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_channel_send: ERROR no such channel");
        return 1;
    }
...
//    }
    ...
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_channel_send: xrdp_sec_send failed");
        return 1;
    }
                           ...
                           int total_data_len, int flags)
{
    ...
    if (session != 0)
    {
        ...
        {
            LOG_DEVEL(LOG_LEVEL_WARNING, "xrdp_channel_call_callback: (BUG?) null callback session->callback");
        }
    }
    else
    {
        LOG_DEVEL(LOG_LEVEL_WARNING, "xrdp_channel_call_callback: (BUG?) null session");
    }
                             ...
                             struct stream *s)
{
    ...
    switch (flags & 3)
    {
        ...
        case 1:
            ...
            free_stream(self->s);
            make_stream(self->s);
            LOG_DEVEL(LOG_LEVEL_TRACE, "xrdp_channel_process_drdynvc - freeing old stream and initializing new stream buffer of size %d bytes", total_length);
        ...
        case 2:
            ...
            break;
        case 3:
            LOG_DEVEL(LOG_LEVEL_TRACE, "xrdp_channel_process_drdynvc: reading command from client");
            ls = s;
            break;
        default:
            LOG(LOG_LEVEL_ERROR, "xrdp_channel_process_drdynvc: "
                        "unknown flag %x", (int) (flags & 3));
            return 1;
    }
    if (ls == NULL)
    {
        LOG_DEVEL(LOG_LEVEL_ERROR, "xrdp_channel_process_drdynvc: ERROR (bug): ls must not be NULL");
        return 1;
    }
    ...
    LOG_DEVEL(LOG_LEVEL_TRACE, "xrdp_channel_process_drdynvc: return value = %d", rv);
    return rv;
}
                     ...
                     int chanid)
{
    ...
    if (channel == NULL)
    {
        LOG(LOG_LEVEL_DEBUG, "xrdp_channel_process: "
