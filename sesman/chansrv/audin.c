...
audin_send_version(int chan_id)
{
    ...
    LOG_DBG(LOG_LEVEL_INFO, "audin_send_version:");
    make_stream(s);
    init_stream(s, 32);
...
audin_send_formats(int chan_id)
{
    ...
    LOG_DBG(LOG_LEVEL_INFO, "audin_send_formats:");
    ...
    for (index = 0; index < num_formats; index++)
    {
        ...
        LOG_DBG(LOG_LEVEL_INFO, "audin_send_formats: sending format wFormatTag 0x%4.4x "
...
audin_send_open(int chan_id)
{
    ...
    LOG_DBG(LOG_LEVEL_INFO, "audin_send_open:");
    make_stream(s);
    init_stream(s, 8192);
...
audin_process_version(int chan_id, struct stream *s)
{
    ...
    LOG_DBG(LOG_LEVEL_INFO, "audin_process_version:");
    if (!s_check_rem(s, 4))
    {
        LOG_DBG(LOG_LEVEL_ERROR, "audin_process_version: parse error");
        return 1;
    }
...
audin_process_formats(int chan_id, struct stream *s)
{
    ...
    LOG_DBG(LOG_LEVEL_INFO, "audin_process_formats:");
    cleanup_client_formats();
    if (!s_check_rem(s, 8))
    {
        LOG_DBG(LOG_LEVEL_ERROR, "audin_process_formats: parse error");
        return 1;
    }
    ...
    for (index = 0; index < num_formats; index++)
    {
        ...
        LOG_DBG(LOG_LEVEL_INFO, "audin_process_formats: recved format wFormatTag 0x%4.4x "
            "nChannels %d nSamplesPerSec %d",
            wf->wFormatTag, wf->nChannels, wf->nSamplesPerSec);
        if (wf->cbSize > 0)
        {
...
audin_process_open_reply(int chan_id, struct stream *s)
{
    ...
    if (!s_check_rem(s, 4))
    {
        LOG_DBG(LOG_LEVEL_ERROR, "audin_process_open_reply: parse error");
        return 1;
    }
...
{
    LOG_DBG(LOG_LEVEL_DEBUG, "audin_process_incoming_data:");
    return 0;
}
...
audin_process_data(int chan_id, struct stream *s)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "audin_process_data: data_bytes %d", data_bytes);
...
{
    LOG_DBG(LOG_LEVEL_INFO, "audin_process_format_change:");
    if (!s_check_rem(s, 4))
    {
        LOG_DBG(LOG_LEVEL_ERROR, "audin_process_format_change: parse error");
        return 1;
    }
    in_uint32_le(s, g_current_format);
    LOG_DBG(LOG_LEVEL_INFO, "audin_process_format_change: g_current_format %d",
        g_current_format);
    return 0;
}
...
audin_process_msg(int chan_id, struct stream *s)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "audin_process_msg:");
    if (!s_check_rem(s, 1))
    {
        LOG_DBG(LOG_LEVEL_ERROR, "audin_process_msg: parse error");
        return 1;
    }
    in_uint8(s, code);
    LOG_DBG(LOG_LEVEL_DEBUG, "audin_process_msg: code %d", code);
    switch (code)
    {
        ...
        default:
            LOG_DBG(LOG_LEVEL_ERROR, "audin_process_msg: unprocessed code %d", code);
            break;
    }
    return 0;
}
...
{
    LOG_DBG(LOG_LEVEL_INFO, "audin_open_response: creation_status 0x%8.8x", creation_status);
...
{
    LOG_DBG(LOG_LEVEL_INFO, "audin_close_response:");
    g_audin_chanid = 0;
    cleanup_client_formats();
    free_stream(g_in_s);
    g_in_s = NULL;
...
{
    LOG_DBG(LOG_LEVEL_DEBUG, "audin_data_first:");
    if (g_in_s != NULL)
    {
        LOG_DBG(LOG_LEVEL_ERROR, "audin_data_first: warning g_in_s is not nil");
        free_stream(g_in_s);
    }
    make_stream(g_in_s);
...
audin_data(int chan_id, char *data, int bytes)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "audin_data:");
...
audin_init(void)
{
    LOG_DBG(LOG_LEVEL_INFO, "audin_init:");
...
audin_deinit(void)
{
    LOG_DBG(LOG_LEVEL_INFO, "audin_deinit:");
    return 0;
}
...
audin_start(void)
{
    ...
    LOG_DBG(LOG_LEVEL_INFO, "audin_start:");
    if (g_audin_chanid != 0)
    {
        return 1;
    }
    ...
    LOG_DBG(LOG_LEVEL_ERROR, "audin_start: error %d g_audin_chanid %d", error, g_audin_chanid);
    return error;
}
...
audin_stop(void)
{
    LOG_DBG(LOG_LEVEL_INFO, "audin_stop:");
