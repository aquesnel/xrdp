                 ...
                 int total_length)
{
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "invalid component type in response; expected 0x%x got 0x%x",
    ...
    switch (pktID)
    {
        ...
        default:
            LOG_DBG(LOG_LEVEL_ERROR, "got unknown response 0x%x", pktID);
            break;
    }
                                   ...
                                   tui32 completion_id)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "device_id=%d path=\"%s\""
                                  ...
                                  int pad_len)
{
    ...
    xstream_free(s);
    LOG_DBG(LOG_LEVEL_DEBUG, "sent close request; expect CID_FILE_CLOSE");
    return 0;
}
    ...
    for (i = 0; i < num_caps; i++)
    {
        ...
        switch (cap_type)
        {
            case CAP_GENERAL_TYPE:
                LOG_DBG(LOG_LEVEL_DEBUG, "got CAP_GENERAL_TYPE");
                break;

            case CAP_PRINTER_TYPE:
                LOG_DBG(LOG_LEVEL_DEBUG, "got CAP_PRINTER_TYPE");
                g_is_printer_redir_supported = 1;
                break;

            case CAP_PORT_TYPE:
                LOG_DBG(LOG_LEVEL_DEBUG, "got CAP_PORT_TYPE");
                g_is_port_redir_supported = 1;
                break;

            case CAP_DRIVE_TYPE:
                LOG_DBG(LOG_LEVEL_DEBUG, "got CAP_DRIVE_TYPE");
            ...
            case CAP_SMARTCARD_TYPE:
                LOG_DBG(LOG_LEVEL_DEBUG, "got CAP_SMARTCARD_TYPE");
...
devredir_proc_client_devlist_announce_req(struct stream *s)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "num of devices announced: %d", device_count);
    ...
    for (i = 0; i < device_count; i++)
    {
        ...
        switch (device_type)
        {
            case RDPDR_DTYP_FILESYSTEM:
                ...
                LOG_DBG(LOG_LEVEL_DEBUG, "device_type=FILE_SYSTEM device_id=0x%x dosname=%s "
            ...
            case RDPDR_DTYP_SMARTCARD:
                ...
                LOG_DBG(LOG_LEVEL_DEBUG, "device_type=SMARTCARD device_id=0x%x dosname=%s",
...
devredir_proc_client_devlist_remove_req(struct stream *s)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "num of devices removed: %d", device_count);
    {
        for (i = 0; i < device_count; i++)
        {
...
devredir_proc_device_iocompletion(struct stream *s)
{
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "IRP with completion ID %d not found", CompletionId);
    }
    else 
    if (irp->callback)
    {
    ...
    else
    {
            ...
            (IoStatus == STATUS_NO_SUCH_FILE && comp_type == CID_LOOKUP))
        {
            ...
            LOG_DBG(LOG_LEVEL_DEBUG, "got %s", completion_type_to_str(comp_type));
        }
        else
        {
        ...
        default:
            LOG_DBG(LOG_LEVEL_ERROR, "got unknown CompletionID: DeviceId=0x%x "
                                 ...
                                 enum NTSTATUS IoStatus)
{
    ...
    if (IoStatus == STATUS_SUCCESS)
    {
        ...
        for (i = 0 ; i < Length ; ++i)
        {
            ...
            //LOG_DBG(LOG_LEVEL_DEBUG, "LastAccessTime:    0x%llx", LastAccessTime);
            //LOG_DBG(LOG_LEVEL_DEBUG, "LastWriteTime:     0x%llx", LastWriteTime);
            //LOG_DBG(LOG_LEVEL_DEBUG, "EndOfFile:         %lld", EndOfFile);
            //LOG_DBG(LOG_LEVEL_DEBUG, "FileAttributes:    0x%x", FileAttributes);
            //LOG_DBG(LOG_LEVEL_DEBUG, "FileNameLength:    %d", FileNameLength);
            LOG_DBG(LOG_LEVEL_DEBUG, "FileName:          %s", filename);
                         ...
                         const char *path)
{
    ...
    if ((irp = devredir_irp_with_pathnamelen_new(strlen(path) + 2)) != NULL)
    {
        ...
        LOG_DBG(LOG_LEVEL_DEBUG, "looking for device_id=%d path=%s", device_id, irp->pathname);
                      ...
                      const char *path)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "fusep=%p", fusep);
    ...
    if ((irp = devredir_irp_with_pathname_new(path)) != NULL)
    {
        ...
        LOG_DBG(LOG_LEVEL_DEBUG, "lookup for device_id=%d path=%s CompletionId=%d",
                           ...
                           tui32 to_set)
{
    ...
    if ((irp = devredir_irp_with_pathname_new(filename)) != NULL)
    {
        ...
        LOG_DBG(LOG_LEVEL_DEBUG, "lookup for device_id=%d path=%s",
                     ...
                     const char *path, int mode)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "device_id=%d path=%s mode=0%o", device_id, path, mode);
                   ...
                   const char *path, int flags)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "device_id=%d path=%s flags=0%x",
    ...
    if ((irp = devredir_irp_with_pathname_new(path)) != NULL)
    {
        ...
        {
            case O_RDONLY:
                LOG_DBG(LOG_LEVEL_DEBUG, "open file in O_RDONLY");
                DesiredAccess = DA_FILE_READ_DATA | DA_SYNCHRONIZE;
                break;

            case O_WRONLY:
                LOG_DBG(LOG_LEVEL_DEBUG, "open file in O_WRONLY");
            ...
            default:
                 ...
                 */
                LOG_DBG(LOG_LEVEL_DEBUG, "open file in O_RDWR");
                        ...
                        tui32 FileId)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "entered: fusep=%p device_id=%d FileId=%d",
    ...
    {
        LOG_DBG(LOG_LEVEL_ERROR, "no IRP found with FileId = %d", FileId);
        return -1;
    }
#endif
                    ...
                    const char *buf, int Length, tui64 Offset)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "DeviceId=%d FileId=%d Length=%d Offset=%lld",
                         ...
                         const char *new_name)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "device_id=%d old_name=%s new_name=%s",
...
devredir_proc_cid_rename_file(IRP *irp, enum NTSTATUS IoStatus)
{
    ...
    if (IoStatus != STATUS_SUCCESS)
    {
        LOG_DBG(LOG_LEVEL_DEBUG, "rename returned with IoStatus=0x%x", IoStatus);
...
static void lookup_read_basic_attributes(IRP *irp, struct stream *s_in)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "processing FILE_BASIC_INFORMATION");
    ...
    //LOG_DBG(LOG_LEVEL_DEBUG, "LastAccessTime:    0x%llx",
    //          (unsigned long long)LastAccessTime);
    //LOG_DBG(LOG_LEVEL_DEBUG, "LastWriteTime:     0x%llx",
    //          (unsigned long long)LastWriteTime);
    //LOG_DBG(LOG_LEVEL_DEBUG, "ChangeTime:        0x%llx",
    //          (unsigned long long)ChangeTime);
    //LOG_DBG(LOG_LEVEL_DEBUG, "FileAttributes:    0x%x", (unsigned int)FileAttributes);
...
{
    tui64 EndOfFile;
    LOG_DBG(LOG_LEVEL_DEBUG, "processing FILE_STD_INFORMATION");
    xstream_seek(s_in, 8);  /* AllocationSize */
    xstream_rd_u64_le(s_in, EndOfFile);
    //LOG_DBG(LOG_LEVEL_DEBUG, "EndOfFile:         %lld",
...
{
    LOG_DBG(LOG_LEVEL_DEBUG, "Lookup with completion_id=%d returning 0x%x",
                         ...
                         enum NTSTATUS IoStatus)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "entry state is %d",irp->gen.lookup.state);
    if (IoStatus != STATUS_SUCCESS)
    {
        /* This is common to all setattr states */
        LOG_DBG(LOG_LEVEL_DEBUG, "last lookup returned with IoStatus=0x%08x", IoStatus);
    ...
    else
    {
        ...
        switch(irp->gen.lookup.state)
        {
            ...
            case E_LOOKUP_CHECK_BASIC:
                ...
                {
                    LOG_DBG(LOG_LEVEL_ERROR, "Expected FILE_BASIC_INFORMATION length"
                              "%d, got len=%d",
            ...
            case E_LOOKUP_CHECK_EOF:
                ...
                {
                    LOG_DBG(LOG_LEVEL_ERROR, "Expected FILE_STD_INFORMATION length"
                              "%d, got len=%d",
...
#define TO_SET_BASIC_ATTRS (TO_SET_MODE | \
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "entry state is %d",irp->gen.setattr.state);
    if (IoStatus != STATUS_SUCCESS)
    {
        /* This is common to all setattr states */
        LOG_DBG(LOG_LEVEL_DEBUG, "last setattr returned with IoStatus=0x%08x", IoStatus);
    ...
    else
    {
        ...
        switch(irp->gen.setattr.state)
        {
            ...
            case E_SETATTR_CHECK_EOF:
                ...
                {
                    LOG_DBG(LOG_LEVEL_ERROR, "Expected FILE_END_OF_FILE_INFORMATION length"
                              "%d, got len=%d",
