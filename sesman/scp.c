...
scp_process_start(void *sck)
{
    ...
    LOG_DBG(LOG_LEVEL_DEBUG, "started scp thread on socket %d", scon.in_sck);
    ...
    switch (scp_vXs_accept(&scon, &(sdata)))
    {
        case SCP_SERVER_STATE_OK:
            ...
            if (sdata->version == 0)
            {
                ...
                LOG_DBG(LOG_LEVEL_DEBUG, "accept ok, go on with scp v0");
                scp_v0_process(&scon, sdata);
            }
            else
            {
                LOG_DBG(LOG_LEVEL_DEBUG, "accept ok, go on with scp v1");
